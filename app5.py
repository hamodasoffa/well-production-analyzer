...
if uploaded_file:
    try:
        ...
        if len(selected_parameters) == 0:
            ...
            # PDF export requires kaleido in requirements.txt
            try:
                pdf_bytes = fig.to_image(format="pdf", width=1600, height=max(900, 420 * len(selected_parameters)), scale=2)

                st.download_button(
                    label="Download Plot PDF",
                    data=pdf_bytes,
                    file_name=f"{well_name}_production_plot.pdf",
                    mime="application/pdf",
                )
            except Exception as pdf_error:
                st.warning("PDF export needs kaleido. Add kaleido to requirements.txt then redeploy.")
